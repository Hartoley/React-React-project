...
const Admin = () => {
  // const {} = useSelector ( state => state.admins) 
  ...
  const formik = useFormik({
    ...
    onSubmit:(value)=>{
      ...
      if (existingadmin) {
        ...
        axios.post(`${endpoint}/admin/register`, value)
        .then((res)=>{
            console.log("Admin signed up successful");
            toast.success("Admin signed up successful")
            navigate("/adminlogin")
        }).catch((error)=>{
          console.log(error);
